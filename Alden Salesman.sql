...
select s.salesman_id,s.name,count(o.customer_id) from salesman s,orders o where s.salesman_id=o.salesman_id group by s.salesman_id,s.name having count(customer_id)>1;


-- 3.List all the salesman and indicate those who have and don't have customers in their cities (Use UNION operation)

select name,'exists' as same_city from salesman s where city in(select city from customer where s.salesman_id=salesman_id)union select name,'not exists' as same_city from salesman s where city not in(select city from customer where s.salesman_id=salesman_id)
